...
filename='users.txt'	# Name of the file where userdata is stored
...
while read -r my_user my_pass; do
	...
	if [ $create_users -eq 1 ]; then
		...
		echo "alias of2412='source /opt/OpenFOAM-v2412/etc/bashrc'" >> /home/$my_user/.bashrc
		echo "shopt -s direxpand" >> /home/$my_user/.bashrc
	else
